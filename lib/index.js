...
export class LedgerBridge extends EventEmitter {
   ...
   */
  constructor(iframe = null, bridgeOverride = BRIDGE_URL, type = 'u2f') {
    super();
    this.bridgeUrl = bridgeOverride + '?' + type;
    this.iframe = iframe === null ? _setupIframe(this.bridgeUrl) : iframe;
  } // ==============================
...
export function makeCardanoBIP44Path(account, chain, address) {
  return [HARDENED + 44, HARDENED + COIN_TYPE, HARDENED + account, chain, address];
}
